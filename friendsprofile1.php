...
while($row = mysql_fetch_array($result))
  ...
  echo $_SESSION['SESS_FIRST_NAME']." ". $row["LastName"] ;
  		...
  		//echo '<a href=cmember-index.php?id=' . $row["friends_id"] . '>' . $row['firstname'] . '</a><br />';
...
include("config.php");
	...
	<ul id="sddm1">
	<li><a href="#" onmouseover="mopen('m3')" onmouseout="mclosetime()"><font color="#0000FF">Wall</font></a>
		<div id="m3" onmouseover="mcancelclosetime()" onmouseout="mclosetime()">	  </div>
	</li><br />
	<li><?php
	if (isset($_GET['id']))
...
while($row = mysql_fetch_array($result))
  ...
  echo '<a href=friendsinfo.php?id=' . $row["friends_id"] . '>' . "Info" .'</a>';
...
while($row = mysql_fetch_array($result))
  ...
  echo '<a href=friendsprofile1.php?id=' . $row["friends_id"] . '>' . "Friends" . '   ('. $numberOfRows . ')' . '</a>';
